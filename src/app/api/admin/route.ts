...
import { isAdminName, verifyAdmin } from '@/lib/admin';
...
export async function DELETE(request: Request) {
  try {
    ...
    if (targetGuest && isAdminName(targetGuest.first_name, targetGuest.last_name)) {
...
export async function PATCH(request: Request) {
  try {
    ...
    if (action === 'edit_guest') {
      ...
      const isAdminTarget = isAdminName(existingGuest.first_name, existingGuest.last_name);
      if (isAdminTarget) {
